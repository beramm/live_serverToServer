        ...
        while True:
            async for response in session.genai_session.receive():
                ...
                try:
                    ...
                    # logger.info(f"checking input and output transcription for {response}")
                    # # Handle input transcription
                    # if getattr(response.server_content, "inputTranscription", None):
                    #     logger.info("received input transcription")
                    #     await websocket.send(json.dumps({
                    #         "type": "input_transcription",
                    #         "data": response.inputTranscription.text
                    #     }))
                    #     continue

                    # # Handle output transcription
                    # if getattr(response.server_content, "outputTranscription", None):
                    #     await websocket.send(json.dumps({
                    #         "type": "output_transcription",
                    #         "data": response.outputTranscription.text
                    #     }))
                    #     continue
...
async def process_server_content(websocket: Any, session: SessionState, server_content: Any):
    ...
    logger.debug(f"server_content dict: {server_content.dict() if hasattr(server_content, 'dict') else server_content}")
    ...
    if server_content.input_transcription:
        logger.info(f"Received input audio transcription from Gemini: {server_content.input_transcription.text}")
        await websocket.send(json.dumps({
            "type": "input_transcription",
            "data": server_content.input_transcription.text
        }))
    if server_content.output_transcription:
        logger.info(f"Received output audio transcription from Gemini: {server_content.output_transcription.text}")
        await websocket.send(json.dumps({
            "type": "output_transcription",
    ...
    if server_content.model_turn:
        ...
        for part in server_content.model_turn.parts:
            if part.inline_data:
                ...
                logger.info(f"Sending text part to client: {part.text}")
                await websocket.send(json.dumps({
                    "type": "text",
                    "data": part.text
                }))
